...
export default class DefaultDatabaseProxy implements DatabaseProxy {
    ...
    async listPortfolios(userId: string): Promise<any[]> {
        const portfolios = await this.portfolioCollection
    ...
    async getUser(filter: ResourceFilter<User>): Promise<any | undefined> {
        const users = await this.userCollection
    ...
    async getUserCredentials(filter: ResourceFilter<UserCredentials>): Promise<UserCredentials | undefined> {
        const userCredentials = await this.userCredentialCollection
        return (userCredentials.findOne(filter) as UserCredentials)
    }
    
    private async provideDatabaseClient(): Promise<MongoClient> {
        console.log(process.env.CRYPTO_TRADING_PLATFORM_PORT);
        const url = encodeURI("mongodb://localhost:27017")
    ...
    private async provideDatabaseCollection<T>(database: string, collection: string): Promise<Collection> {
        const client = await this.client
